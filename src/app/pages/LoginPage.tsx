...
export function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const cleanPwd = sanitizePassword(password);
      const data = await login(cleanEmail, cleanPwd);
            ...
            <>
              ...
              <form onSubmit={handleCambiar} className="space-y-4">
                ...
                <button type="submit" disabled={loading}
                  ...
                  {loading ? <><Loader2 size={15} className="animate-spin" /> Guardando...</> : "Establecer contraseña y continuar"}
                </button>
              </form>
            </>
          )}
        </div>
        <p className="text-center text-xs text-slate-600 mt-6">SCCA · Sistema de Control de Calidad del Agua</p>
      </div>
    </div>
  );
}
